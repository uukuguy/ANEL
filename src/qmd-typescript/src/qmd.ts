...
import { ENV_EMIT_SPEC, ENV_DRY_RUN } from "./anel/index.js";
import { getSpecForCommand } from "./anel/spec.js";
...
function parseCLI() {
  const { values, positionals } = parseArgs({
    ...
    options: {
      ...
      // ANEL options
      "emit-spec": { type: "boolean" },  // output JSON Schema and exit
      "dry-run": { type: "boolean" },    // validate parameters but don't execute
    },
  ...
  // ANEL flags (CLI flags or environment variables)
  const emitSpec = !!values["emit-spec"] || !!process.env[ENV_EMIT_SPEC];
  const dryRun = !!values["dry-run"] || !!process.env[ENV_DRY_RUN];
...
function showHelp(): void {
  ...
  console.log("");
  console.log("ANEL options (Agent-Native Execution Layer):");
  console.log("  --emit-spec                - Output command JSON Schema and exit");
  console.log("  --dry-run                  - Validate parameters but don't execute");
  console.log("");
...
if (import.meta.main) {
  ...
  // ANEL: handle --emit-spec for any command
  if (cli.emitSpec) {
    const spec = getSpecForCommand(cli.command);
    if (spec) {
      console.log(JSON.stringify(spec, null, 2));
      process.exit(0);
    }
    // If no spec found, fall through to normal execution
  }

  switch (cli.command) {
    case "context": {
      if (cli.dryRun) {
        const sub = cli.args[0] || "list";
        console.log(`[DRY-RUN] Would execute context ${sub}`);
        if (sub === "add") console.log(`[DRY-RUN] Args: ${cli.args.slice(1).join(" ")}`);
        if (sub === "rm") console.log(`[DRY-RUN] Path: ${cli.args[1]}`);
        break;
      }
    ...
    case "get": {
      if (cli.dryRun) {
        console.log(`[DRY-RUN] Would get file: ${cli.args[0]}`);
        console.log(`[DRY-RUN] From: ${cli.values.from || 0}, Limit: ${cli.values.l || "all"}`);
        break;
      }
      if (!cli.args[0]) {
    ...
    case "multi-get": {
      if (cli.dryRun) {
        console.log(`[DRY-RUN] Would multi-get pattern: ${cli.args[0]}`);
        console.log(`[DRY-RUN] Limit: ${cli.values.l || "all"}, Max-bytes: ${cli.values["max-bytes"] || DEFAULT_MULTI_GET_MAX_BYTES}`);
        break;
      }
      if (!cli.args[0]) {
    ...
    case "collection": {
      if (cli.dryRun) {
        const sub = cli.args[0] || "list";
        console.log(`[DRY-RUN] Would execute collection ${sub}`);
        if (sub === "add") console.log(`[DRY-RUN] Path: ${cli.args[1] || "."}, Name: ${cli.values.name || "auto"}, Mask: ${cli.values.mask || DEFAULT_GLOB}`);
        if (sub === "remove" || sub === "rm") console.log(`[DRY-RUN] Name: ${cli.args[1]}`);
        if (sub === "rename" || sub === "mv") console.log(`[DRY-RUN] Old: ${cli.args[1]}, New: ${cli.args[2]}`);
        break;
      }
      const subcommand = cli.args[0];
      switch (subcommand) {
    ...
    case "status":
      if (cli.dryRun) {
        console.log("[DRY-RUN] Would show index status");
        break;
      }
      showStatus();
      break;
    ...
    case "update":
      if (cli.dryRun) {
        console.log(`[DRY-RUN] Would update index`);
        console.log(`[DRY-RUN] Pull: ${!!cli.values.pull}`);
        break;
      }
    ...
    case "embed":
      if (cli.dryRun) {
        ...
        console.log(`[DRY-RUN] Force: ${!!cli.values.force}`);
        break;
      }
    ...
    case "search":
      if (cli.dryRun) {
        console.log(`[DRY-RUN] Would execute BM25 search for query: ${cli.query}`);
        console.log(`[DRY-RUN] Limit: ${cli.opts.limit}, Collection: ${cli.opts.collection || "all"}`);
        break;
      }
      if (!cli.query) {
    ...
    case "vsearch":
      if (cli.dryRun) {
        console.log(`[DRY-RUN] Would execute vector search for query: ${cli.query}`);
        console.log(`[DRY-RUN] Limit: ${cli.opts.limit}, Collection: ${cli.opts.collection || "all"}`);
        break;
      }
      if (!cli.query) {
    ...
    case "query":
      if (cli.dryRun) {
        console.log(`[DRY-RUN] Would execute hybrid search for query: ${cli.query}`);
        console.log(`[DRY-RUN] Limit: ${cli.opts.limit}, Collection: ${cli.opts.collection || "all"}`);
        break;
      }
      if (!cli.query) {
    ...
    case "mcp": {
      if (cli.dryRun) {
        console.log("[DRY-RUN] Would start MCP server");
        console.log("[DRY-RUN] Transport: stdio");
        break;
      }
    ...
    case "cleanup": {
      if (cli.dryRun) {
        console.log("[DRY-RUN] Would cleanup cache, orphaned vectors, inactive documents, and vacuum DB");
        break;
      }
      const db = getDb();
